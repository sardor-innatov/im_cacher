...
func (aof *AOF) readValues(iterator func(v *resp.Value)) error {
	aof.atEnd = false
	if _, err := aof.f.Seek(0, 0); err != nil {
		return err
	}
	rd := resp.NewReader(aof.f)
	for {
...
func (aof *AOF) Append(v resp.Value) error {
	return aof.AppendMulti([]resp.Value{v})
}

func (aof *AOF) AppendDelCommand(key string) {
	// WARNING: this is hardcoded delete command to write into aof,
	// it was made to avoid multiple unnecessary encoding and decoding
	// for improving performance.
	bs := []byte(fmt.Sprintf("*4\r\n$3\r\ndel\r\n:0\r\n+%s\r\n$-1\r\n", key))
	// reqType = "del"
	// ttl = 0
	// key = key (argument)
	// value = null
...
func (aof *AOF) AppendMulti(vs []resp.Value) error {
	var bs []byte
	for _, v := range vs {
		b, err := v.MarshalRESP()
		if err != nil {
...
func (aof *AOF) Scan(iterator func(v *resp.Value)) error {
	aof.mu.Lock()
	defer aof.mu.Unlock()
	if aof.closed {
		return errClosed
